...
- `auth`：登录、获取用户资料、更新资料、修改密码、登出等方法。
- `imageHost`：图床图片、飞书授权和文件夹管理方法。
...
- 业务方法（`auth`、`imageHost`）屏蔽具体 URL 与请求细节，为上层提供语义化接口。

## 数据流
1. 页面或 Provider 调用 `lib/auth`、`lib/imageHost`。
...
async function bootstrap() {
  ...
  const assets = await listImageAssets(10, 0)
